...
*
*   Why is this needed?
*   - In Storel some blocks are superblocks, which output much higher amounts of coins
...
void CMasternodePayments::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // disable all Storel specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f STOREL\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
