...
const char * const BITCOIN_CONF_FILENAME = "storel.conf";
const char * const BITCOIN_PID_FILENAME = "storeld.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "storel" is a composite category enabling all Storel-related debug output
            if(ptrCategory->count(string("storel"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\StorelCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\StorelCore
    // Mac: ~/Library/Application Support/StorelCore
    // Unix: ~/.storelcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "StorelCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/StorelCore";
#else
    // Unix
    return pathRet / ".storelcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty storel.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override storel.conf
